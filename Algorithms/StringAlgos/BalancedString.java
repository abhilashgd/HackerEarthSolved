...
public class BalancedString {
	 ...
	 private void solve() {
	     ...
	     while(N-->0){
	     	...
	     	for(int i=0;i<len;i++)
	     		arr[str.charAt(i)]++;
	     	int sum = Math.min(arr[91], arr[93])+Math.min(arr[123], arr[125])+Math.min(arr[40], arr[41]);
	     	sum*=2;
	     		sb.append(sum+"\n");
	     }
	 ...
	 public boolean isDifference(char curr,char next,char prev){
	  ...
	  public static void main(String[] args) throws Exception { new BalancedString().run(); }
