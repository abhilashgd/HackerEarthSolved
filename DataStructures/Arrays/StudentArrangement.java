...
public class StudentArrangement {
		 ...
		 while(N-->0){
			 int row = nextInt()-1;
				  if(map.get(row)<K)
					  ...
					  boolean found = findAndPut(map,row,M,K);
					  if(!found)
						  count++;
				  	   }
		 }
	...
	private boolean findAndPut(TreeMap<Integer, Integer> map, int row, int m,int k) {
		if(row==m-1){
			for(int i=0;i<m-1;i++){
				 if(map.get(i)<k)
				 {
	...
	public static void main(String[] args) throws Exception { new StudentArrangement().run(); }
