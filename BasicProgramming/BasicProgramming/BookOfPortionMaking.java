...
public class BookOfPortionMaking {
	...
	private void solve() {
		...
		int sum=0;
		if(input.length()<10)
			pw.println("Illegal ISBN");
		else{
		for(int i=1;i<=10;i++)
			sum+=(i*Character.getNumericValue(input.charAt(i-1)));
		if(sum%11==0)
			pw.println("Legal ISBN");
		else
			pw.println("Illegal ISBN");
		}
	}
  ...
  public static void main(String[] args) throws Exception { new BookOfPortionMaking().run(); }
