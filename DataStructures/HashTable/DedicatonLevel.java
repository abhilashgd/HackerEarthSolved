...
public class DedicatonLevel {
	 ...
	 private void solve() {
		 ...
		 while(N-->0)
		 	 points.add(new Point(nextString(),nextInt()));
		 
		 	Collections.sort(points,new Comparator<Point>() {
			 public int compare(Point o1, Point o2) {
				 int result = Integer.compare( o2.getY(),  o1.getY());
		            return result;
			 }});
		 Iterator<Point> pItr = points.iterator();
		 Point node;int count=0;
		 while(pItr.hasNext()){
			 node=pItr.next();
			 pw.println(node.getX());count++;
			 if(count==3)
				 break;
		 }
		}
	  ...
	  public static void main(String[] args) throws Exception { new DedicatonLevel().run(); }
