...
public class LabFileWork {
	 ...
	 private void solve() {
		 ...
		 for(int i=1;i<=N;i++)
			 ll.add(new Points(i,nextInt(),nextInt()));
		 ...
		 Collections.sort(ll,new Comparator<Points>(){
				...
				public int compare(Points o1, Points o2) {
					int result=0;
					result= Integer.compare(o1.sum, o2.sum);
					if(result==0)
	 ...
	 class Points{
			...
			public Points(int no,int ti, int di) {
				this.no=no;
				this.ti=ti;
				this.di=di;
				this.sum=ti+di;
...
class sBuilder{
	...
	void getOutput(){
	  ...
	  public static void main(String[] args) throws Exception { new LabFileWork().run(); }
