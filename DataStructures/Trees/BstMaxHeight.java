...
class BST{
	...
	int heightOfBST(BST node){
		int lHeight,rHeight;
		if(node==null)
			return 0;
		else{
			lHeight=heightOfBST(node.left);
			rHeight=heightOfBST(node.right);
			}
		if(lHeight>rHeight)
			return (lHeight+1);
		else
	  ...
	  public static void main(String[] args) throws Exception { new BstMaxHeight().run(); }
