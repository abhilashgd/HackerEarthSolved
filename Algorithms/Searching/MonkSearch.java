...
public class MonkSearch {
	 ...
	 private void solve() {
		...
		int Q=nextInt();
		while(Q-->0){
			int indicator = nextInt();
			int y = nextInt();
			switch(indicator){
				case 0:
					long temp = Arrays.stream(arr)
	                .filter(x -> x < y ).count()-1;
					pw.println(N-temp);
					break;
				case 1:
					 temp = Arrays.stream(arr)
	                .filter(x -> x <= y ).count()-1;
					pw.println(N-temp);
	  ...
	  public static void main(String[] args) throws Exception { new MonkSearch().run(); }
