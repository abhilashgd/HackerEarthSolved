...
public class HolidaySeason {
	...
	private void solve() {
		 ...
		 String s=nextString();
		 int count=0;
		 TreeSet<Integer>[] al= new TreeSet[CHAR_SIZE];
		 for(int i=0;i<CHAR_SIZE;i++)
			 al[i]=new TreeSet<Integer>();
		 
		 for(int i=0;i<N;i++){al[s.charAt(i)-'a'].add(i);}
		 ...
		 for(int i=0;i<CHAR_SIZE;i++){
			 if(al[i].size()>1){
				Iterator<Integer> itr = al[i].iterator();
				 ...
				 for(int j=xa+1;j<xc;j++){
					 int ch=s.charAt(j)-'a';
					 if(al[ch].ceiling(xc)!=null)
					 {count+=al[ch].tailSet(al[ch].ceiling(xc)).size();}
				 }
				 ...
				 while(itr.hasNext()){
					 ...
					 for(int j=xa+1;j<xc;j++){
						 int ch=s.charAt(j)-'a';
						 if(al[ch].ceiling(xc)!=null){count+=al[ch].tailSet(al[ch].ceiling(xc)).size();}
					 }
				 }
			 }
		 }
	  ...
	  public static void main(String[] args) throws Exception { new HolidaySeason().run(); }
