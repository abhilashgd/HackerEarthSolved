...
public class ChoosingJudges {
	 ...
	 private void solve() {
		...
		for(int i=1;i<=T;i++){
			 int N=nextInt();
			 if(N==1)
				 pw.println("Case "+i+": "+nextInt());
			 else{
				 int[] arr = new int[N];
				 for(int j=0;j<N;j++)
					 arr[j]=nextInt();
				 int max = knapSack(arr,N);
				 pw.println("Case "+i+": "+max);
			 }
		 }
			 
	 }
	  private int knapSack(int[] arr, int n) {
		int max=0;
		int in=arr[0];
		int out =0;
		int outN;
	...
	public static void main(String[] args) throws Exception { new ChoosingJudges().run(); }
